...
struct NutrimentItem {
    ...
    var nutritionTableRow: NutritionTableRow? {
        ...
        if let perServing = perServing {
            perServingValue = String(format: "%.2f \(unit)", perServing)
        }
        return NutritionTableRow(
            label: localized.localizedString,
            perSizeValue: String(format: "%.2f \(unit)", per100g),
            perServingValue: perServingValue,
            highlight: isMainItem)
    }
